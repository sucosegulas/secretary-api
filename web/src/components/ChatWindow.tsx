...
import { Send, CheckCircle, Eye, Smartphone } from 'lucide-react';
...
export default function ChatWindow({ chat, onSend, onResolve, instances }: ChatWindowProps) {
  const [instanceMode, setInstanceMode] = useState<'bot' | 'monitor'>('bot');

  useEffect(() => {
    const inst = instances[chat.instanceId];
    if (inst) setInstanceMode(inst.mode || 'bot');
  }, [chat.instanceId, instances]);

  const isMonitor = instanceMode === 'monitor';
  ...
  return (
    <div className="flex flex-col h-full bg-[#efeae2] relative shadow-inner">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">
              {isMonitor ? (
                <span className="text-blue-600 font-semibold flex items-center gap-1"><Eye size={12} /> Monitorando</span>
      ...
      <div className="bg-white p-4 border-t border-slate-200 z-10">
        {isMonitor ? (
          <div className="max-w-3xl mx-auto text-center text-sm text-blue-600 bg-blue-50 px-5 py-3 rounded-2xl font-medium">
            <Eye size={16} className="inline mr-1.5" />
            Modo Monitor — apenas visualização, não é possível enviar mensagens
          </div>
        ) : (
